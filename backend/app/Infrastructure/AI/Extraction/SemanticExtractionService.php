...
- approx_year — примерный год (число) ТОЛЬКО если пользователь явно назвал год, десятилетие или возраст на момент события; НЕ выводи год из контекста и догадок
- occurred_at — ISO-дата (YYYY-MM-DD), если известна точная дата
- life_period — жизненный период («детство», «школа», «1990-е» и т.п.), если год неизвестен — предпочитай это поле вместо approx_year
- temporal_source — "user_stated" если пользователь явно назвал время; "ai_inferred" только если год назван прямо в тексте, но не пользователем (редко)
...
Хронологические связи (если пользователь подтвердил порядок): precedes (A раньше B), follows (A позже B), during (событие внутри эпохи).
Для epoch создавай главы жизни; события привязывай к эпохам через part_of или during.
