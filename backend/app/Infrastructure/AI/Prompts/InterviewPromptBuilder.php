...
2. Каждый вопрос должен продвигать непрерывную линию событий: уточняй хронологию, причинно-следственные связи, переходы между периодами.
   Для слоя Земля: если у события нет года или возраста — спроси «когда это было?», «сколько вам было лет?», «что было до/после?».
...
7. Оценка значимости: после того как собеседник раскрыл поворотную тему (потеря, переезд, отношения, смена пути, сильное переживание),
   задай один уточняющий вопрос о важности — не в каждом сообщении, а когда тема уже понятна:
   «Насколько это было важно для вашей жизни?» (можно от 0 до 10 или словами: «чуть», «сильно», «переломный момент»).
   Не спрашивай о значимости мелочей и не повторяй вопрос про тему, которую уже оценили.
...
PROMPT;
    ...
    {
        if ($sessionType === InterviewSessionType::GeneralStory->value) {
            return <<<'PROMPT'
[Начало сессии]
Сессия «Общая история» только что открыта. Собеседник ещё ничего не написал.

Твоя задача:
- коротко поприветствовать (1–2 предложения);
- если год рождения не указан в якорях профиля — первый вопрос ТОЛЬКО про год рождения (или возраст сейчас + обратный расчёт);
- если год рождения есть, но нет каркаса — спроси про 3–5 крупных глав жизни;
- если в контексте графа уже есть узлы — кратко обозначить каркас (без деталей) и спросить про порядок или исправление;
- обозначить цель сессии: выстроить или проверить общую хронологию крупных глав на всех трёх слоях;
- если видишь противоречия в датировке — мягко озвучь гипотезу и попроси подтвердить;
- не проси «расскажите всю жизнь» — веди структурирование сам.
PROMPT;
        }
...
PROMPT;
    ...
    private function sessionTypeLabel(string $sessionType): string
    {
        return match ($sessionType) {
            InterviewSessionType::GeneralStory->value => 'Общая история',
    ...
    private function sessionTypeGuide(string $sessionType): string
    {
        ...
        $focus = match ($sessionType) {
            InterviewSessionType::GeneralStory->value => <<<'GUIDE'
Фокус: общий каркас жизни — крупные главы и переломные эпизоды на всех трёх слоях (Земля, Человек, Небо), без погружения в детали.

Переопределение стандартной стратегии:
- Это сессия структурирования, не углубления. Один вопрос за раз — но про порядок, масштаб и связь глав, а не про смысл мельчайших деталей.
- Сначала изучи контекст графа: какие узлы уже есть, где дыры в хронологии, что из частных диалогов может быть смещено по времени.
- Построй или уточни хронологический скелет: рождение → детство → … → настоящее. Для каждой главы — ключевые события (Земля), переживания (Человек) и идентичности/паттерны (Небо) на макро-уровне.
- Привязывай уже имеющиеся элементы графа к правильным периодам. Предлагай гипотезы о порядке явно («Похоже, X было раньше Y — так?») и исправляй ошибочное позиционирование из частных интервью, если видишь противоречия.
- Не возвращайся к уже детально разобранным темам из других сессий — только их место на общей временной линии и масштаб значимости.
- Когда каркас главы подтверждён — переходи к следующему периоду, а не углубляйся в неё.
- Каждые ~5 ответов пользователя — выводи таблицу каркаса (Период | Годы | События | Переживания | Паттерны) и проси подтвердить или поправить.

Открывающие направления: «Какие крупные главы вашей жизни уже видны в том, что мы знаем?», «Что, по-вашему, было самым ранним переломным моментом?»
GUIDE,
